...
public final class TypeSkipper {
    ...
    private static void skip(Instruction instruction, RecordingStream stream) throws IOException {
        switch (instruction) {
            case VARINT, CP_ENTRY -> stream.readVarint();
            case BYTE -> stream.skip(1);
            case FLOAT -> stream.skip(4);
            case DOUBLE -> stream.skip(8);
            case STRING -> ParsingUtils.skipUTF8(stream);
        }
    }
}
